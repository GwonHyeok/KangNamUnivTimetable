...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public void setCurrentTextColor(int position, int tabsColor) {
        ControlSharedPref settingepref = new ControlSharedPref(getContext(), "Setting.pref");
        boolean IS_DARK_THEME = settingepref.getValue(MainAppSettingActivity.TTB_THEME, 0) == 1 ? true : false;
        int tab_text_color = getResources().getColor(R.color.tab_text);

        if (IS_DARK_THEME) {
            tab_text_color = getResources().getColor(R.color.tab_text_dark);
        }

        // Tabs TextView Color Reset
        for(int i=0; i<tabCount; i++) {
            View v = tabsContainer.getChildAt(i);
            if(v instanceof TextView) {
                TextView tab = (TextView) v;
                tab.setTextColor(tab_text_color);
                tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize);
            }
        }
        // Selected Tab TextView Set Color
        View v = tabsContainer.getChildAt(position);
        if (v instanceof  TextView) {
            TextView tab = (TextView) v;
            tab.setTextColor(tabsColor);
            tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize+8);
        }
    }
