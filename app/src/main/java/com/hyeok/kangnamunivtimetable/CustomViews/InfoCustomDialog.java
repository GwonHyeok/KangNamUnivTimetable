...
import com.hyeok.kangnamunivtimetable.Activity.CampusMap;
import com.hyeok.kangnamunivtimetable.Activity.ExtraTime;
import com.hyeok.kangnamunivtimetable.Activity.MapsActivity;
...
public class InfoCustomDialog extends Dialog {
    ...
    public InfoCustomDialog(Context context, final int position) {
        super(context);
        mContext = context;
        ...
        // 배경화면 레이아웃 초기화
//        RelativeLayout bg_layout = (RelativeLayout) findViewById(R.id.extratime_main_layout);
        // 제목 설정
        ...
        Button cancelbtn = (Button) findViewById(R.id.dialog_cancelbutton);
                                ...
                                switch (Map_index) {
                                    case 0:
                                        try {
                                            ...
                                            String tmp_uri = null;
                                            if (geo.equals("경")) {
                                                tmp_uri = appUtils.GEO_GYEONCHEON;
                                            } else if (geo.equals("이")) {
                                                tmp_uri = appUtils.GEO_EGONG;
                                            } else if (geo.equals("천")) {
                                                tmp_uri = appUtils.GEO_CHEONN;
                                            } else if (geo.equals("교")) {
                                                tmp_uri = appUtils.GEO_GOYUK;
                                            } else if (geo.equals("후")) {
                                                tmp_uri = appUtils.GEO_HUSENG;
                                            } else if (geo.equals("우")) {
                                                tmp_uri = appUtils.GEO_WUWON;
                                            } else if (geo.equals("예")) {
                                                tmp_uri = appUtils.GEO_YESUL;
                                            } else if (geo.equals("목")) {
                                                tmp_uri = appUtils.GEO_MOKYANG;
                                            } else if (geo.equals("인")) {
                                                tmp_uri = appUtils.GEO_INSA;
                                            } else if (geo.equals("승")) {
                                                tmp_uri = appUtils.GEO_SEUNGLEE;
                                            } else if (geo.equals("샬")) {
                                                tmp_uri = appUtils.GEO_SHALROM;
                                            } else if (geo.equals("운")) {
                                                tmp_uri = appUtils.GEO_UNDONG;
                                            }
                                            Intent intent = new Intent(mContext, MapsActivity.class);
                                            intent.putExtra("geo", tmp_uri);
                                            intent.putExtra("class_name", appUtils.getFullClassName(geo));
                                            mContext.startActivity(intent);
